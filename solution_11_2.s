...
_main:
    ...
    mov x27, #-1         ; out id
    adrp x0, svr_id@PAGE
    add x0, x0, svr_id@PAGEOFF
    mov x1, #-1
    str x1, [x0]
    adrp x0, dac_id@PAGE
    add x0, x0, dac_id@PAGEOFF
    str x1, [x0]
    adrp x0, fft_id@PAGE
    add x0, x0, fft_id@PAGEOFF
    str x1, [x0]
...
get_id:
    adrp x9, current_code@PAGE
    add x9, x9, current_code@PAGEOFF
    str x0, [x9]
...
found_node:
    adrp x12, current_code@PAGE
    add x12, x12, current_code@PAGEOFF
    ldr x12, [x12]
    adrp x11, code_svr@PAGE
    add x11, x11, code_svr@PAGEOFF
    ldr x11, [x11]
    cmp x12, x11
    b.ne check_dac_id
    adrp x11, svr_id@PAGE
    add x11, x11, svr_id@PAGEOFF
    str x10, [x11]
    b id_done

check_dac_id:
    adrp x11, code_dac@PAGE
    add x11, x11, code_dac@PAGEOFF
    ldr x11, [x11]
    cmp x12, x11
    b.ne check_fft_id
    adrp x11, dac_id@PAGE
    add x11, x11, dac_id@PAGEOFF
    str x10, [x11]
    b id_done

check_fft_id:
    adrp x11, code_fft@PAGE
    add x11, x11, code_fft@PAGEOFF
    ldr x11, [x11]
    cmp x12, x11
    b.ne id_done
    adrp x11, fft_id@PAGE
    add x11, x11, fft_id@PAGEOFF
    str x10, [x11]
...
count_paths:
    ...
    adrp x2, dac_id@PAGE
    add x2, x2, dac_id@PAGEOFF
    ldr x2, [x2]
    cmp x19, x2
    b.ne check_fft_visit
    orr x26, x26, #1

check_fft_visit:
    adrp x2, fft_id@PAGE
    add x2, x2, fft_id@PAGEOFF
    ldr x2, [x2]
    cmp x19, x2
    b.ne check_out_node
    orr x26, x26, #2
...
check_memo:
    mov x2, #4
    madd x25, x19, x2, x26
    adrp x2, memo_valid@PAGE
    add x2, x2, memo_valid@PAGEOFF
    ldrb w1, [x2, x25]
