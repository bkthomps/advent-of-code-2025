...
connect_shortest:
    ...
    mov x21, x20         ; component count
    mov x23, #0          ; pair index
...
sort_pairs:
    adrp x9, pair_dist@PAGE
    add x9, x9, pair_dist@PAGEOFF
    adrp x10, pair_a@PAGE
    add x10, x10, pair_a@PAGEOFF
    adrp x11, pair_b@PAGE
    add x11, x11, pair_b@PAGEOFF
    mov x4, x19
    mov x12, x4
    lsr x12, x12, #1

gap_loop:
    cbz x12, sort_pairs_done
    mov x13, x12
...
shell_j_loop:
    ...
    ldr x3, [x11, x0, lsl #3]
    str x1, [x9, x17, lsl #3]
    str x2, [x10, x17, lsl #3]
    str x3, [x11, x17, lsl #3]
    mov x17, x0
    b shell_j_loop

shell_insert:
    str x14, [x9, x17, lsl #3]
    str x15, [x10, x17, lsl #3]
    str x16, [x11, x17, lsl #3]
    add x13, x13, #1
    b shell_i_loop
...
input_path:
    .asciz "input_08.txt"
open_error:
    .asciz "failed to open input_08.txt\n"
